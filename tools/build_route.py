...
four things go stale at once, so they are all rewritten here in one pass rather
than by hand in four places that can drift apart:
  ...
  4. m/o — trail mile and miles-off — on every bundled POI  (data/pois-nearby.json)
...
def read_gpx(path):
    """Every trkpt in the file, in file order."""
...
def main():
    ...
    # route swap has a single number to move.
    src = src.replace(
